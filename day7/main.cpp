...
Positions parse(std::istream& in) {
  Positions result;
  std::string s{};
...
namespace part2 {
  Result solve_for(char const* pData) {
    ...
    // Part 2 - try the naive way...
    std::vector<size_t> distance_sums{};
    for (int i=*min_iter;i<*max_iter;i++) {
      size_t distance_sum = 0;
      for (int j = 0; j<positions.size();j++) {
