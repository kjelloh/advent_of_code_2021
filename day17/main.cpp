...
bool hits_target(Area const& target, int dx, int dy) {
  // std::cout << "\nmax_y_if_target_hit dx:" << dx << " dy:" << dy;
  bool result{ false };
  bool found_one = false;
  int x = dx;
...
namespace part2 {
  Result solve_for(char const* pData) {
      ...
      std::stringstream in{ pData };
      auto target_area = parse(in);
      int min_dx = 1; // Trial and error ;)
      int max_dx = 175; // Trial and error ;)
      int min_dy = -98; // Trial and error ;)
      int max_dy = 98; // Trial and error ;)
      // search the space of dx,dy
      size_t hit_count{ 0 };
      int max_y{ 0 };
      for (int dx = min_dx; dx < max_dx; dx++) {
        for (int dy = min_dy; dy < max_dy; dy++) {
          //std::cout << "\ntest init{dx:" << dx << ",dy:" << dy << "}";
          if (hits_target(target_area, dx, dy)) {
            ++hit_count;
            // std::cout << "\n\thit_count=" << hit_count;
          }
        }
